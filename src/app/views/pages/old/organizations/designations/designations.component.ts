...
export class DesignationsComponent implements OnInit {
  ...
  onSubmit(event: Event): void {
    ...
    this.http.post(`${this.API_URL}/designations`, this.currentRecord).subscribe({
      next: (response) => {
  ...
  updateRecord(event: Event): void {
    ...
    this.http.put(`${this.API_URL}/designations/${this.currentRecord.id}`, this.currentRecord).subscribe({
      next: (response) => {
        this.isLoading = false;
        this.fetchCities();
        this.activeModal?.close();
  ...
  deleteSelectedRecords(): void {
    if (confirm('Are you sure you want to permanent delete the selected record(s)?')) {
      ...
      const deleteRequests = ids.map(id => this.http.delete(`${this.API_URL}/designations/${id}`).toPromise());
