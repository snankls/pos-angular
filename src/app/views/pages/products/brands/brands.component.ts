...
export class BrandsComponent implements OnInit {
      ...
      next: (response) => {
        ...
        this.rows = response.map(item => ({
          ...
          image_url: item.image_url
            ? `${this.IMAGE_URL}/uploads/brands/${item.image_url}`
  ...
  deleteSelectedRecords(): void {
    if (confirm('Are you sure you want to permanent delete the selected record(s)?')) {
      ...
      const deleteRequests = ids.map(id => this.http.delete(`${this.API_URL}/brands/${id}`).toPromise());
  ...
  deleteRecord(row: any): void {
    if (confirm(`Are you sure you want to delete "${row.name}"?`)) {
      this.http.delete(`${this.API_URL}/brands/${row.id}`).subscribe({
        next: () => {
